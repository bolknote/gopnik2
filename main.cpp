...
#warning Unimplemented textmode
  ...
  cur_game->add_hero_phrase(ht_index, "Чё, мент поганый, на меня уставился? Хорька ищешь?", "Ах ты змеёныш! Я те, сука, ща поговорю...");
  cur_game->add_enemy_phrase(ht_index, "Оба-на! Вот тебя-то, гопаря хренова, я и искал...", "Ща пойдёшь больницу искать!");
  ...
  cur_game->add_hero_phrase(ht_index, "Э, скин, ты по жизни такой урод был или только когда побрился?", "А ну заткнул пасть, говно! Да я из тебя ща калеку сделаю!");
  ...
  cur_game->add_hero_phrase(ht_index, "Интересно, вот если тебя, эмо, отпинать, будешь ли ты ещё хуёвей выглядеть?", "Делай, что хочешь, только не трогай чёлку!");
  cur_game->add_enemy_phrase(ht_index, "Ты тру эмо или позер?", "Я антиэмо, чувак. И для тебя это не самый лучший поворот событий.");

  ht_index = cur_game->add_hero_type("Интеллигент", 0, 2, 2, 4, 4, 0, 5, 0, 5, 0, 0, 0, 2, 2, 0);

  cur_game->add_hero_phrase(ht_index, "Эй, четырёхглазый, а где тут библиотека?", "Для таких ублюдков, как ты, библиотека навсегда закрыта!");
  cur_game->add_enemy_phrase(ht_index, "Слышь, маргинал, это не ты вчера нассал возле памятника Чернышевскому?", "А? Чё? Кто такой Чернышевский?");
  ...
  cur_game->add_buy_phrase(pl_index, plm_index, "Док: Что-то я тебе не те таблетки дал... Ладно, не поспишь пару деньков");
  ...
  cur_game->add_buy_phrase(pl_index, plm_index, "Док: Что, опять шов разошёлся?! В следующий раз со своими нитками придёшь!");

  plm_index = cur_game->add_price_list_memb(pl_index, "Починить переломы", "", 7);

  cur_game->add_buy_phrase(pl_index, plm_index, "Док: Ого! Охренеть у тебя переломы! Как ты только по дороге сюда не сдох");
  ...
  cur_game->add_buy_phrase(pl_index, plm_index, "Дубинку прикупил?! Слушай, а ведь с тобой уже опасно связываться...");
  ...
  cur_game->add_buy_phrase(pl_index, plm_index, "Сухари оказались жёстче, чем ты думал - пришлось пожертововать двумя зубами");

  plm_index = cur_game->add_price_list_memb(pl_index, "Разливное пиво 0.5 л.", "прибавляет 15з", 8);

  cur_game->add_buy_phrase(pl_index, plm_index, "В который раз ты подумал - какую же всё-таки мочу продают в ларьках");
  ...
  cur_game->add_buy_phrase(pl_index, plm_index, "Э, бармен, а чё у тя коктейль тормозной жидкостью отдаёт?\nА, так и надо? Тогда без базара...");
  ...
  cur_game->add_buy_phrase(pl_index, plm_index, "Чего-то у меня в глазах потемнело. Водка точно из этилового спирта?");
  ...
  cur_game->add_buy_phrase(pl_index, plm_index, "Кассирша: Скока-скока жетонов? Ебать тя в рот, не можешь громче сказать!");
  cur_game->add_buy_phrase(pl_index, plm_index, "Кассирша: Бля, ты б мне ещё копеечными монетами отсыпал!");
  cur_game->add_buy_phrase(pl_index, plm_index, "Кассирша: Сдачу тебе ещё надо? Умный что ли, считать умеешь? Видел у входа\n          бак мусора? Так вот завтра мы таких умников, как ты, закопаем\n          туда и вывезем на грузовиках. Следующий, ёпт!");
  cur_game->add_buy_phrase(pl_index, plm_index, "Кассирша: Глухой, что ли! Ещё раз повторяю - бери жетон и вали! Чё сказал?\n          Тебя не спросила, урод, когда мне включать микрофон. Следующий!");

  plm_index = cur_game->add_price_list_memb(pl_index, "Проездной", "", 80);

  cur_game->add_buy_phrase(pl_index, plm_index, "Кассирша: Чё, раздолбай, деньги появились? Бери свой проездной и что б больше\n          я тебя возле кассы не видела!");
  ...
  plm_index = cur_game->add_price_list_memb(pl_index, "Барсетка", "увеличивает полученную понтовость в притоне в 2 раза", 40);
