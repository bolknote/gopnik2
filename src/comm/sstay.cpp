...
int sstay() {
    ...
    const std::string mess[] = {
            ...
            "-Здарова, братан! Как ты? Прикинь, \"Чёрную речку\" открыли, хочу глянуть как да чо.\n",
    ...
    // Станция «Чёрная речка» ещё не открыта, мы «на районе» и сменили уже третий район
    if (!cur_game->stn[5].avail && cur_game->active_loc == 0 && main_hero->district > 2 && CHANCE(1, 200)) {
    ...
    if (
        ...
        if (cur_game->stay_mh == 1) {
            if (
                    (cur_game->active_loc == 1) &&
                    (cur_game->enemy->is_type(HeroType::VETER))) {
                std::cout << mess[2];
                cur_game->set_loc(0);
            }
        ...
        if (cur_game->stay_v > 0) {
            if (cur_game->active_loc == 1) {
                if (cur_game->stay_v == 1) {
                    ...
                    ht_index = cur_game->search_ht(main_hero->station ? HeroType::BRATVA_S_OBSCHAGI : HeroType::BRATVA);
                    ...
                    lads->set_name(HeroType(HeroType::BRATVA).ToString());
