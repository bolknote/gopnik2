...
int k ()
{
  ...
  if (cur_game->open_str == 2)
  {
    ...
    settextattr (14);
    printw (mess [18], cur_game->enemy->get_type (), cur_game->enemy->get_level ());
  }
  // end стрела #1
  // -------------
  ...
  if (cur_game->active_loc == 1)
  {
    ...
    if (cur_game->num_k > 4)
    {
      ...
      {
        settextattr (14);
        printw ("Голоса из толпы: %s", audience_mess [getrandom (0, 9)]);
      }
    }
  }          

  empty_k_count 
    = main_hero->get_level () % 10 + main_hero->get_min_empty_kick_count () + main_hero->district;          
     ...
     )
  {
    ...
    for (i = 0; i < enemy->inv_amount; i++)
    {
      ...
      printw (mess [3], main_hero->inv [inv_index].name);
...
*/
    ...
    {
      main_hero->add_money (getrandom (cur_game->pl [0].members [1].price - main_hero->get_money () - 1, cur_game->pl [0].members [1].price));
    }

    if (enemy->get_ciga () > 0)
    {
      printw (mess [4], enemy->get_type ());
    ...
    if (cur_game->get_open_hp () == 2)
    {
      ...
      settextattr (9);
      printw (mess [11], enemy->get_max_health ());
    }
