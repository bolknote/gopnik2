...
int help()
{
  ...
  {
  case 0:
    printf("%s", "  Чтобы прокачаться, ты должен увеличивать свои навыки. Изначально навыки в\nсумме составляют 12, поэтому вероятность увеличения навыка при получении нового\nуровня понтовости составляет отношение начальной величины этого навыка к 12.\n");
    printf("%s", "  Всего навыков четыре штуки:\n");
    printf("%s", "    Сила      - увеличивает урон и добавляет 1 здоровья\n");
    printf("%s", "    Ловкость  - +5% попадания, если больше 90% то можно бить дважды\n");
    printf("%s", "    Живучесть - 5 здоровья\n");
    printf("%s", "    Удача     - будет чаще везти по жизни\n");
    printf("%s", "  Навыки влияют на следующие характеристики:\n");
    printf("%s", "    Здоровье = 10+Живучесть*5+Сила\n");
    printf("%s", "    Урон     = (Сила/2)мин - (Сила)макс урона который нанесёшь\n");
    printf("%s", "    Точность = 20+Ловкость*5\n");
    printf("%s", "    Броня    - насколько уменьшается сила вражеского удара\n");
    printf("%s", "  Навыки и характеристики могут также повышаться с помощью различных вещей,\nкоторые будут обнаруживаться по ходу игры.\n\n");
    printf("%s", "  На районе есть много разных мест, куда можно зайти на досуге (рынок, притон и\nт.д.) и которые могут давать ряд преимуществ и возможностей. Помни, что при\nсмене района тебе заново придётся осваивать эти места!\n\n");
    printf("%s", "  Ну и напоследок, при смене режима или локации не забывай просматривать новые\nкоманды (для этого нужно набрать i).\n");
    break;

  case 1:
  case 12:
    printf("%s", "  Сейчас ты находишься в режиме битвы. Находится ты тут можешь по двум\nпричинам - либо ты захотел кого-то прикончить, либо кто-то захотел\nприкончить тебя, третьего не дано.\n");
    printf("%s", "  Твоя основная задача на данный момент - выжить. Если вдруг совсем\nприжмёт, то можешь глотнуть пивка или выкурить косячок, что б не сдохнуть.\nТакже можешь позвать подмогу, если у тебя имеется мобила и есть знакомые\nкореша в этом районе.\n");
    printf("%s", "  В случае отпина врага ты получаешь опыт в размере суммы его навыков.\nКроме того, к тебе переходят все его вещи (это могут быть оружие, какие-то\nартефакты, хлам или деньги).\n");
    printf("%s", "  Помни, что помимо снижения здоровья битва черевата переломами челюсти\n(не сможешь жрать, пить и курить) и ноги (не сможешь наносит двойных\nударов).\n");
    break;

  case 2:
    printf("%s", "  У ветеринара ты можешь поправить здоровье. Главное помни, что всё\nстоит денег. Так что, если нет бабла, смысла идти сюда нет никакого.\n");
    break;

  case 3:
  case 11:
    printf("%s", "  На рынке ты можешь приобрести разного рода вещи, либо потискать\nкошельки у лохов - тут главное не попасться, а то могут возникнуть\nбольшие проблемы.\n");
    break;

  case 4:
    printf("%s", "  В притоне тусуются реальные пацаны, много чего можно сделать. Заходи\nсюда почаще. Главное, если дела пойдут плохо, сможешь позвать на помощь\nсвоих корешей - следи за притонной понтовостью.\n");
    break;

  case 5:
    printf("%s", "  Всё то, что нельзя купить на рынке, можно купить у барыг. Также имеется\nвозможность спихнуть им награбленный хлам и продать ненужные вещи.\n");

    if (main_hero->station == 0)
    {
      printf("%s", "  У районных барыг, как правило, не особо большой выбор. Говорят, что\nпо-настоящему здравые вещи можно пробить у барыг на Невском. Правда и\nзаплатить за них придётся немалую цену.\n");
    }
    break;

  case 6:
    printf("%s", "  Если тебе нужны бабки, то ты всегда можешь продать ненужные вещи. Помни,\nчто после продажи выкупить ты уже ничего не сможешь!\n");

    if (main_hero->station == 0)
    {
      printf("%s", "  Правда, районные барыги за них могут дать не так много, как хотелось бы.\nХочешь наварить побольше деньжать - толкай вещи у барыг на Невском.\n");
    }
    break;

  case 7:
    printf("%s", "  В качалке ты можешь поднять свои навыки. Даже если сейчас ты самый\nкрутой, то не факт, что завтра ты останешься таким же. Так что почаще\nсюда заглядывай.\n");
    break;

  case 8:
    printf("%s", " В клубе ты можешь как следуе отдохнуть: потусить на танцполе или\nотдохнуть за кружкой пива в баре.");

    if (main_hero->station)
    {
      printf("%s", " Также можешь попробовать\nзакадрить девчонку (сможешь вписываться у неё на хате, чтобы поправить\nздоровье).");
    }
  ...
  case 9:
    printf("%s", "  Благодаря метро, ты имеешь возможность выбираться из своих ебеней в\nцентр города.\n");
    break;

  case 13:
    printf("%s", "  В слэме ты можешь офигенно прокачать свою ловкость. Чем жёстче слэм,\nтем больше ловкости ты можешь прокачать, но тем опаснее это будет для\nтвоего здоровья.\n");
    break;
  }
